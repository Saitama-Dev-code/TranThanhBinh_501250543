...
## 🔧 Công nghệ sử dụng
- **Backend**: PHP 8.1+ (Pure PHP, OOP MVC)
...
## 🔒 Bảo mật (Security)
- **CSRF Token**: Bảo vệ các form (Đăng ký, Đăng nhập, Quên mật khẩu) khỏi lỗi giả mạo yêu cầu chéo.
- **Mã hóa Mật khẩu**: Sử dụng `Bcrypt` thông qua hàm `password_hash()` để đảm bảo an toàn.
- **Ngăn chặn XSS**: Các dữ liệu do người dùng nhập đều được filter qua `htmlspecialchars()`.
- **SQL Injection**: Sử dụng thư viện PDO với Prepared Statements (`bindParam`) trong `BaseModel`.
- **Quản lý Upload File**: Kiểm tra MIME-type và định dạng chặt chẽ để chống upload script độc hại.
- **Phân quyền (RBAC)**: Tách riêng cấu trúc file của Admin, kiểm tra quyền `role > 0` tự động.
...
-`password` (VARCHAR(255), Nullable): Mật khẩu (Đã băm bằng Bcrypt. Cho phép Null nếu đăng nhập bằng mạng xã hội)
...
-`reset_token_expires` (DATETIME, Nullable): Thời gian hết hạn của token phục hồi mật khẩu
